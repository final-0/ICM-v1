...
class ICA(CompressionModel):
    def __init__(self, config=[2, 2, 2, 2, 2, 2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0, N=128,  M=320, num_slices=4, max_support_slices=5, **kwargs):
        ...
        self.m_down3 = [ConvTransBlock(dim, dim, self.head_dim[2], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW')
                      for i in range(config[2])] + \
                      [conv3x3(2*N, num_slices*64, stride=2)]###
        ...
        self.g_s = nn.Sequential(*[ResidualBlockUpsample(320, 2*N, 2)] + self.m_up1 + self.m_up2 + self.m_up3)
        ...
        #)
        self.h_a = nn.Sequential(
            *[ResidualBlockWithStride(num_slices*64, 2*N, 2)] + \
            self.ha_down1
        )

        self.hs_up1 = [ConvTransBlock(N, N, 32, 4, 0, 'W' if not i%2 else 'SW') 
                      for i in range(config[3])] + \
                      [subpel_conv3x3(2*N, num_slices*64, 2)]###
        ...
        self.atten_mean = nn.ModuleList(
            nn.Sequential(
                SWAtten((num_slices*64 + (num_slices*64//self.num_slices)*min(i, 5)), (num_slices*64 + (num_slices*64//self.num_slices)*min(i, 5)), 16, self.window_size,0, inter_dim=128)###
        ...
        self.cc_mean_transforms = nn.ModuleList(
            nn.Sequential(
                conv(num_slices*64 + (num_slices*64//self.num_slices)*min(i, 5), 224, stride=1, kernel_size=3),
                nn.GELU(),
                conv(224, 128, stride=1, kernel_size=3),
                nn.GELU(),
                conv(128, (num_slices*64//self.num_slices), stride=1, kernel_size=3),
            ) for i in range(self.num_slices)
        )
        ...
        self.lrp_transforms = nn.ModuleList(
            nn.Sequential(
                conv(num_slices*64 + (num_slices*64//self.num_slices)*min(i+1, 6), 224, stride=1, kernel_size=3),
                nn.GELU(),
                conv(224, 128, stride=1, kernel_size=3),
                nn.GELU(),
                conv(128, (num_slices*64//self.num_slices), stride=1, kernel_size=3),
    ...
    def forward(self, x, yy):
        ...
        y_slices = y_add.chunk(self.num_slices, 1)
        y_hat_slices = []
        y_likelihood = []
        mu_list = []
        scale_list = []
        """
        mean_support = latent_means
        mean_support = self.atten_mean[0](mean_support)
        mu = self.cc_mean_transforms[0](mean_support)
        mu = mu[:, :, :y_shape[0], :y_shape[1]]
        mu_list.append(mu)
        #print(mu.size())
        scale_support =latent_scales
        scale_support = self.atten_scale[0](scale_support)
        scale = self.cc_scale_transforms[0](scale_support)
        scale = scale[:, :, :y_shape[0], :y_shape[1]]
        scale_list.append(scale)
        #print(scale.size())
        _, y_slice_likelihood = self.gaussian_conditional(y_add, scale, mu)
        y_likelihood.append(y_slice_likelihood)
        y_add_hat = ste_round(y_add - mu) + mu

        lrp_support = torch.cat([mean_support, y_add_hat], dim=1)
        lrp = self.lrp_transforms[0](lrp_support)
        lrp = 0.5 * torch.tanh(lrp)
        y_add_hat += lrp
        #print(y_add_hat.size())
        ...
        y_add_hat = torch.cat(y_hat_slices, dim=1)
        means = torch.cat(mu_list, dim=1)
        scales = torch.cat(scale_list, dim=1)
        
        yy_dash = yy[:,0:self.num_slices*64,:,:]+y_add_hat
        yy[:,0:self.num_slices*64,:,:] = yy_dash

        # tz = torch.zeros(y_hat_slice.size()).cuda()
        # y_add_hat = torch.cat((y_add_hat,tz),dim=1)
        # yy = y_add_hat
        # yy[:,64*4:64*5,:,:] = tz
        # yy[:,64*3:64*4,:,:] = tz
        # yy[:,64*2:64*3,:,:] = tz
